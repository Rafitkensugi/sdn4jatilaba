...
<head>
    ...
    <title>Kelola Artikel - SDN 4 Jatilaba</title>
...
<body class="bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex min-h-screen">
        <!-- Sidebar Component -->
        <x-sidebar active-menu="berita"></x-sidebar>
            ...
            <main class="p-6">
                ...
                <div class="bg-gradient-to-r from-indigo-600 to-purple-700 dark:from-indigo-800 dark:to-purple-900 rounded-2xl shadow-xl p-6 mb-8">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p class="text-indigo-100 text-sm sm:text-base">Kelola artikel dan berita SDN 4 Jatilaba</p>
                        </div>
                        <a href="{{ route('admin.artikel.create') }}" 
                           class="inline-flex items-center px-5 py-2.5 bg-white hover:bg-gray-50 text-indigo-600 font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300">
                        <div class="flex items-center">
                            <div class="p-3 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                                <i class="fas fa-newspaper text-indigo-600 dark:text-indigo-400 text-xl"></i>
                            </div>
                            <div class="ml-4">
                                <p class="text-gray-500 dark:text-gray-400 text-sm font-medium">Total Artikel</p>
                                <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ $artikels->total() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <p class="text-gray-500 dark:text-gray-400 text-sm font-medium">Artikel Bulan Ini</p>
                                <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ $artikels->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300">
                        <div class="flex items-center">
                            <div class="p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
                                <i class="fas fa-image text-blue-600 dark:text-blue-400 text-xl"></i>
                            </div>
                            <div class="ml-4">
                                <p class="text-gray-500 dark:text-gray-400 text-sm font-medium">Dengan Gambar</p>
                                <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ $artikels->whereNotNull('gambar')->count() }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 transform hover:scale-105 transition-all duration-300">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-3xl font-bold text-gray-800 dark:text-white">{{ $artikels->currentPage() }}/{{ $artikels->lastPage() }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="relative group">
                                            <img src="{{ asset('storage/'.$artikel->gambar) }}" 
                                                 ...
                                                 onclick="showImageModal('{{ asset('storage/'.$artikel->gambar) }}', '{{ $artikel->judul }}')">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-semibold text-gray-900 dark:text-white line-clamp-2">{{ $artikel->judul }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-600 dark:text-gray-400">{{ \Carbon\Carbon::parse($artikel->tanggal)->format('d M Y') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <form action="{{ route('admin.artikel.destroy', $artikel->id) }}" method="POST" class="inline-block" onsubmit="return confirmDelete()">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-xl font-semibold text-gray-500 dark:text-gray-400 mb-2">Belum Ada Artikel</p>
                                            <p class="text-gray-400 dark:text-gray-500 mb-4">Mulai tambahkan artikel pertama Anda</p>
                                            <a href="{{ route('admin.artikel.create') }}" class="inline-flex items-center px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
                                                <i class="fas fa-plus-circle mr-2"></i>
                ...
                <div class="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-all duration-300">
                        <!-- Image -->
                        @if($artikel->gambar)
                        <div class="relative h-40 overflow-hidden group cursor-pointer" onclick="showImageModal('{{ asset('storage/'.$artikel->gambar) }}', '{{ $artikel->judul }}')">
                            <img src="{{ asset('storage/'.$artikel->gambar) }}" 
                                 alt="{{ $artikel->judul }}" 
                            ...
                            <div class="absolute top-3 left-3 bg-indigo-600 text-white px-3 py-1 rounded-full text-xs font-bold">
                                #{{ $loop->iteration + ($artikels->currentPage() - 1) * $artikels->perPage() }}
                            </div>
                        </div>
                        @else
                        ...
                        <div class="p-4 sm:p-5">
                            ...
                            <div class="flex gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <form action="{{ route('admin.artikel.destroy', $artikel->id) }}" method="POST" class="flex-1" onsubmit="return confirmDelete()">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    <div class="col-span-1 md:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 sm:p-12 text-center">
                        <i class="fas fa-newspaper text-gray-400 text-6xl sm:text-7xl mx-auto mb-6"></i>
                        <p class="text-xl sm:text-2xl font-bold text-gray-700 dark:text-gray-300 mb-3">Belum Ada Artikel</p>
                        <p class="text-gray-500 dark:text-gray-400 mb-6">Mulai tambahkan artikel pertama Anda</p>
                        <a href="{{ route('admin.artikel.create') }}" class="inline-flex items-center px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
                            <i class="fas fa-plus-circle mr-2"></i>
                            Tambah Artikel Pertama
                        </a>
                    </div>
